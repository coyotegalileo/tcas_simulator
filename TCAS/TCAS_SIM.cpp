...
void to_display::sendMsgList(int sockfd, struct sockaddr_in* sockadd, std::vector< to_display > display_l)
{
	...
	struct iovec io[5*display_l.size()];
	...
	message.msg_iov=io;
	message.msg_iovlen=5*display_l.size();
...
{	
	...
	double x_enu, y_enu, z_enu; // tirar
	...
	for(int i=0;i<airspace.size();i++)
	{
		...
		{
			to_display item(d_alt*m_to_ft, bearing, distance,v_speed*m_to_ft*60,0);			
		...
		{
			to_display item(d_alt*m_to_ft, bearing, distance,v_speed*m_to_ft*60,1);
			display_l.push_back(item);
			printf("TA\n");
		}
		...
		{
			to_display item(d_alt*m_to_ft, bearing, distance,v_speed*m_to_ft*60,2);
			display_l.push_back(item);
			printf("PT\n");
		}
		...
		{
			to_display item(d_alt*m_to_ft, bearing, distance,v_speed*m_to_ft*60,3);
			display_l.push_back(item);
			printf("OT\n");
		}
...
int main( int argc, char *argv[] )
{
 ...
 //    to_display SIM2DISP;
 //    SIM2DISP.newSocks( &sockfd_disp, &serv_addr_disp, port_disp, false);
